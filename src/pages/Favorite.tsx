      ...
      <Layout>
        <div className="grid grid-cols-4 gap-3 p-3">
          {this.state.loading
            ...
            : this.state.datas.map((data) => <Card key={data.id} title={data.title} image={data.poster_path} id={data.id} labelButton="REMOVE FROM FAVORITE" onClickFav={() => this.removeFavorite(data)} />)}
        </div>
      </Layout>
    );
  }
