    ...
    <Layout>
      ...
      <div className="grid grid-cols-4 gap-3 p-3">
        {loading
          ...
          : datas.map((data) => <Card key={data.id} title={data.title} image={data.poster_path} id={data.id} labelButton="ADD TO FAVORITE" onClickFav={() => handleFavorite(data)} />)}
      </div>
      <div className="btn-group w-full justify-center">
        ...
        <button className="btn" onClick={() => nextPage()} disabled={page === totalPage}>
          »
        </button>
      </div>
    </Layout>
